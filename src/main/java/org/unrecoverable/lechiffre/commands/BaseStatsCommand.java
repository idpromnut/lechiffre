 ...
 *
 */
public class BaseStatsCommand extends BaseCommand implements IStatsCommand {
	...
	public List<User> searchForUser(String searchName) {
		...
		for(IGuild lGuild: guildStatsMap.keySet()) {
			...
			searchForUser(searchName, lGuild).forEach(u -> {
				lUsers.add(lGuildStats.getUsers().get(u.getStringID()));	
			});
		}
		return lUsers;
	}
